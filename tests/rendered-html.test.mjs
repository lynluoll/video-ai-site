...
test("server-renders the complete advertising strategy", async () => {
  ...
  assert.match(wppHtml, /WPP \/ HAVAS.*代理商是最大的增长入口.*Creative.*Production/s);
  assert.match(wppHtml, /模型 API 为主/);
  assert.match(wppHtml, /WPP Open.*80,000\+ 员工/s);
  assert.match(wppHtml, /短期 · 2026.*30%.*70%.*中长期.*10% 至 15%.*70%/s);
  ...
  assert.match(wppHtml, /PRODUCTION.*10% 至 15%.*70%.*3D \/ Digital Twin.*一致性.*可控镜头.*4K.*低闪烁/s);
  assert.match(wppHtml, /MEDIA.*20% 至 30%.*Master Video.*渠道、市场、语言与人群.*精准编辑.*本地化.*投放反馈/s);
...
test("keeps responsive safeguards for desktop, tablet, phone, and narrow phone", async () => {
  ...
  assert.match(css, /\.wppAgencyStrategy \{[^}]*grid-template-columns: minmax\(330px, \.82fr\) minmax\(0, 1\.18fr\)/);
  assert.match(css, /\.wppWorkstreamGrid \{[^}]*grid-template-columns: repeat\(3, minmax\(0, 1fr\)\)/);
  assert.match(css, /@media \(max-width: 1120px\) \{[\s\S]*?\.wppWorkstreamGrid \{ grid-template-columns: 1fr; \}/);
  assert.match(css, /@media \(max-width: 760px\) \{[\s\S]*?\.wppPenetrationPlan \{ grid-template-columns: 1fr; \}/);
