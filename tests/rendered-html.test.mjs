...
test("covers every Bojie requirement in the page source", async () => {
  ...
  assert.match(page, /Vision LLM · Seed 2\.0/);
...
test("keeps responsive safeguards for desktop, tablet, phone, and narrow phone", async () => {
  ...
  assert.match(css, /\.brandSolutionBody \{[^}]*grid-template-columns: minmax\(360px, \.52fr\) minmax\(0, 1\.48fr\)/);
  assert.match(css, /\.brandOriginalFlow \{[^}]*grid-template-columns: minmax\(150px, \.82fr\)/);
  assert.match(css, /\.brandDemoCompactGrid \{[^}]*grid-template-columns: repeat\(2, minmax\(0, 1fr\)\)/);
